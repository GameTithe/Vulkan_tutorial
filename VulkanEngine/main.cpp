...
//model path
const std::string MODEL_PATH = "models/bike/neo tokyo.obj";
const std::string TEXTURE_PATH = "textures/bike/neo_NeoTokyoBike_color.png";
...
private:
	...
	{
		VkDeviceSize bufferSize = sizeof(UnifromBufferObject);
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			vkMapMemory(device, uniformBuffersMemory[i], 0, MAX_FRAMES_IN_FLIGHT, 0, &uniformBuffersMapped[i]);
		}
	...
	{
		imageAbailableSemaphores.resize(MAX_FRAMES_IN_FLIGHT);
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAbailableSemaphores[i]) != VK_SUCCESS ||
				vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS ||
				vkCreateFence(device, &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS)
			{
				throw std::runtime_error("failed to create semphore");
			}
		}
	}
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		{
			throw std::runtime_error("failed tot record command buffer");
		}
	}
	...
	void createGraphicsPipeline()
	{
		...
		{
			throw std::runtime_error("failed to create piepline layout");
		}
	...
	void drawFrame()
	{
		...
		VkSemaphore waitSemaphores[] = { imageAbailableSemaphores[currentFrame]};
